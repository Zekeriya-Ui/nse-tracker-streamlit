...
def make_sparkline(data, color="#2ecc71"):
    fill_color = hex_to_rgba(color, 0.15) if color.startswith("#") else "rgba(46,204,113,0.15)"
...
def color_positive_column(col):
    return ["color: #2ecc71" for _ in col]

def color_negative_column(col):
    return ["color: #e74c3c" for _ in col]

def color_change_column(col):
    return [
        "color: #2ecc71" if v > 0 else ("color: #e74c3c" if v < 0 else "")
        for v in col
    ]
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigation",
        ["🏠 Dashboard", "📊 Equities", "🏭 Sectors", "⭐ Watchlist", "📰 News"],
...
if page == "Dashboard":
    ...
    with tab1:
        ...
        styled_gainers = (
            ...
            .apply(color_positive_column, subset=["Change %"])
        )
    ...
    styled_df = (
        ...
        .apply(color_change_column, subset=["Change %"])
    )
    ...
    with c2:
        fig2 = px.bar(
            sector_df.sort_values("Avg Change %"),
            x="Avg Change %",
            y="Sector",
            orientation="h",
    ...
    styled_sector_df = sector_df.style.format({
        "Market Cap": "Ksh {:,.0f}",
        "Avg Change %": "{:+.2f}%",
        "Stocks": "{:.0f}"
    })
    ...
    available_tickers = [t for t in NSE_STOCKS if t not in st.session_state.watchlist]

    if available_tickers:
        add_ticker = st.selectbox("Add to watchlist", available_tickers)
        if st.button("➕ Add"):
            st.session_state.watchlist.append(add_ticker)
            st.rerun()
    else:
        st.info("All available stocks are already in your watchlist.")
    ...
    if not st.session_state.watchlist:
        ...
        for ticker in st.session_state.watchlist:
            ...
            c2.caption(s["name"])
            c3.markdown(
                f'<span style="color:{clr}">Ksh {s["price"]:,.2f} ({sign}{s["change"]}%)</span>',
                unsafe_allow_html=True
            )
    ...
    news_items = [
        {"title": "Safaricom posts strong Q1 results, M-Pesa revenue up 18%", "source": "Business Daily", "time": "2 hours ago", "ticker": "SCOM",
        ...
        {"title": "KCB Group completes acquisition of regional bank, eyes Uganda expansion", "source": "The Standard", "time": "4 hours ago", "ticker": "KCB",
        ...
        {"title": "Kenya Airways receives new Boeing 787 Dreamliner, resumes long-haul routes", "source": "Nation Africa", "time": "6 hours ago", "ticker": "KQ",
        ...
        {"title": "East African Breweries launches new premium spirits line", "source": "Reuters", "time": "8 hours ago", "ticker": "EABL",
        ...
        {"title": "NSE All Share Index hits 5-month high amid foreign investor inflows", "source": "Capital FM", "time": "10 hours ago", "ticker": "NSE",
        ...
        {"title": "Equity Group reports record profits, declares Ksh 4.00 dividend", "source": "Business Daily", "time": "12 hours ago", "ticker": "EQTY",
